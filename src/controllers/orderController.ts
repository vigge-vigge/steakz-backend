...
export const createOrder = async (req: Request, res: Response) => {
    try {
        ...
        if (userRole === 'CUSTOMER') {
            ...
            if (!actualCustomerId) {
                actualCustomerId = userId; // Use cashier ID as placeholder for walk-in orders
            }
        } else {
            return res.status(403).json({ message: 'Only customers and cashiers can create orders' });
        }
